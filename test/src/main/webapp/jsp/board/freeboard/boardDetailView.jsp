...
<div id="main-container">
    ...
    <!-- 게시글 수정 버튼 추가 -->
    <button onclick="location.href='<%= request.getContextPath() %>/board/freeboard/edit?contentNo=<%= boardDto.getContentNo() %>'" class="action-button">수정</button>
    <!-- 게시글 삭제 버튼 추가 -->
    <button onclick="confirmDeletePost(<%= boardDto.getContentNo() %>)" class="action-button">게시글 삭제</button>

    <h3 class="subtitle">댓글 달기</h3>
    <form action="<%=request.getContextPath()%>/addComment" method="post" onsubmit="return validateForm(this);" class="comment-form">
        <input type="hidden" name="contentNo" value="<%=boardDto.getContentNo()%>">
        <input type="hidden" name="boardType" value="freeboard">
        <textarea name="commentContent" rows="4" placeholder="댓글을 입력하세요" class="comment-textarea"></textarea>
        <br>
        <input type="submit" value="댓글 추가" class="submit-button">
    </form>
    ...
    <ul class="comments" id="comment-list">
        <c:forEach var="comment" items="${commentList}">
            <li class="comment">
                ...
                <button onclick="confirmDelete(${comment.commentNo}, <%= boardDto.getContentNo() %>, 'freeboard')" class="action-button">삭제</button>
                <button onclick="openEditForm(${comment.commentNo})" class="action-button">수정</button>
                <!-- 수정 폼 -->
                <div id="editForm-${comment.commentNo}" class="edit-form">
                    <form action="<%=request.getContextPath()%>/editComment" method="post" onsubmit="return validateForm(this);">
                        ...
                        <input type="hidden" name="contentNo" value="<%=boardDto.getContentNo()%>">
                        <input type="hidden" name="boardType" value="freeboard">
                        <textarea name="commentContent" rows="3" class="edit-textarea">${comment.contentComment}</textarea>
    ...
    <% } %>
    <a href="<%=request.getContextPath() %>/board/freeboard/list" class="back-link">목록으로 돌아가기</a>
</div>
