...
public class EditNoticeController extends HttpServlet {
	...
	private static final String UPLOAD_DIRECTORY = "D:/GudiSpring/img/noticeboard";
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			req.getRequestDispatcher("/jsp/admin/notice/editNoticeBoardForm.jsp").forward(req, res);
		} catch (Exception e) {
			e.printStackTrace();
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
					...
					if (item.isFormField()) {
						...
						if (!fileName.isEmpty()) {
							...
							if (!(fileName.endsWith(".png") || fileName.endsWith(".jpg") || fileName.endsWith(".webp")
								...
								filePaths.add("noticeboard/" + uniqueFileName); // 상대 경로로 추가
								   // 본문에 이미지 태그 추가
                                // 본문에 이미지 태그 추가
                                   if (contentText == null) {
                                	   contentText = "";
	        ...
	        // 현재 게시글의 기존 데이터를 가져옴
			NoticeDto existingBoard = boardDao.selectOne(contentNo);
