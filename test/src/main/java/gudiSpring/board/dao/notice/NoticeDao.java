...
public class NoticeDao {
	...
	// 전체 조회 (페이징 적용)
    public List<NoticeDto> selectList(int startRow, int pageSize) throws Exception {
        ...
        try {
        	...
        	sql += "       WHERE CONTENT_BOARD_INFO_NO = 1 "; // 게시판 번호에 따라 수정 필요
            ...
            List<NoticeDto> boardList = new ArrayList<>();
            while (rs.next()) {
                ...
                NoticeDto boardDto = new NoticeDto(contentNo, contentSubject, contentText, contentFiles,
    ...
    public int getTotalCount() throws SQLException {
        String sql = "SELECT COUNT(*) FROM BOARD_CONTENT WHERE CONTENT_BOARD_INFO_NO = 1";
	...
	//게시글 상세 조회 
    public NoticeDto selectOne(int contentNo) throws SQLException {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    return new NoticeDto(contentNo, contentSubject,
...
//selectone종료
	// 게시글 추가
		...
		public void addBoard(NoticeDto noticeDto) throws SQLException {
			// 게시글을 DB에 삽입하는 SQL 문
 ...
 //리뷰그외 다른게시판시반드시수정**
		    ...
		    try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
		        ...
		        try (PreparedStatement keyPstmt = connection.prepareStatement(generatedKeyQuery);
		            ...
		            if (rs.next()) {
		                ...
		                // 이미지 파일 경로들을 DB에 삽입
		                for (String filePath : noticeDto.getContentFiles()) {
	...
	// 게시글 수정 메소드
	public void updateBoard(NoticeDto noticeDto) throws SQLException {
		 String sql = "UPDATE BOARD_CONTENT SET CONTENT_SUBJECT = ?,"
		  ...
		  try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
		        pstmt.setString(1, noticeDto.getContentSubject());
		        pstmt.setString(2, noticeDto.getContentText());
		        pstmt.setInt(3, noticeDto.getContentNo());
	        ...
	        try (PreparedStatement deleteImgPstmt = connection.prepareStatement(deleteImgSql)) {
	            deleteImgPstmt.setInt(1, noticeDto.getContentNo());
	        ...
	        for (String filePath : noticeDto.getContentFiles()) {
	            String imgSql = "INSERT INTO BOARD_CONTENT_IMG (BOARD_IMG_NO, CONTENT_NO, CONTENT_IMG_PATH) "
	                          + "VALUES (BOARD_IMG_NO.NEXTVAL, ?, ?)";
	            try (PreparedStatement imgPstmt = connection.prepareStatement(imgSql)) {
	                imgPstmt.setInt(1, noticeDto.getContentNo());
	                imgPstmt.setString(2, filePath);
	                imgPstmt.executeUpdate();
	            }
	        }
